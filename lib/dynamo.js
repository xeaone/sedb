...
module.exports = class Dynamo {
    ...
    constructor (options = {}) {
        ...
        this.after = options.after ? options.after.bind(this) : null;
        this.before = options.before ? options.before.bind(this) : null;
    ...
    async put (item) {
        ...
        if (this.before) await this.before('put', item);
        ...
        if (this.after) await this.after('put', item);
    }

    async get (item) {
        if (!item) throw new Error('item required');
        if (this.before) await this.before('get', item);
        ...
        if (this.after) await this.after('get', unmarshall);
    ...
    async remove (item, options) {
        if (!item) throw new Error('item required');
        if (this.before) await this.before('remove', item);
        ...
        const unmarshall = Item ? this.unmarshall(Item) : null;
        if (this.after) await this.after('remove', unmarshall);
    ...
    async add (item, options) {
        if (!item) throw new Error('item required');
        if (this.before) await this.before('add', item);
        ...
        await this.dynamo.putItem(data).promise();
        if (this.after) await this.after('add', item);
    }
    ...
    async update (item, options) {
        if (!item) throw new Error('item required');
        if (this.before) await this.before('update', item);
        ...
        if (this.after) await this.after('update', item);
    }
    ...
    async query (condition, options) {
        ...
        if (this.before) await this.before('query', condition);
        ...
        if (this.after) await this.after('query', result);
        return result;
    }

    async scan (condition) {
        if (!condition) throw new Error('condition required');
        if (this.before) await this.before('scan', condition);
        ...
        if (this.after) await this.after('scan', result);
        return result;
    }
