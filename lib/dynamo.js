...
module.exports = class Dynamo {
        ...
        if (this.indexes && this.indexes.length) {
            ...
            for (const index of this.indexes) {
                ...
                data.GlobalSecondaryIndexes.push({
                    ...
                    IndexName: `${index.hash}-${index.range}-index`,
                    ProvisionedThroughput: {
    ...
    async update (item, options) {
        ...
        if (options && 'updateExpression' in options) {
        ...
        if (options && 'conditionExpression' in options) {
            delete ExpressionAttributeNames[ `#${hash}` ];
            delete ExpressionAttributeNames[ `#${range}` ];
    ...
    async query (condition, options) {
        ...
        for (const name in condition) {
            ...
            const index = this.indexes.find(({ hash, range }) => hash === name || range === name);

            if (name === hash || name === range || index) {
                if (index) IndexName = `${index.hash}-${index.range}-index`;

                if (name === range || name === index?.range && options && options.begins === true) {
